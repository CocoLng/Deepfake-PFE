...
class HomePage(QWidget):
    ...
    def init_ui(self):
        ...
        # En-tête principal (Layout vertical pour inclure le logo et le titre séparément)
        header_layout = QVBoxLayout()
        header_layout.setContentsMargins(10, 10, 10, 10)
        header_layout.setSpacing(0)

        # Sous-layout pour le logo (aligné à droite)
        logo_layout = QHBoxLayout()
        logo_layout.setContentsMargins(0, 0, 0, 0)
        sorbonne_logo = QLabel()
        sorbonne_pixmap = QPixmap("Sorbonne.png").scaled(250, 250, Qt.KeepAspectRatio, Qt.SmoothTransformation)
        sorbonne_logo.setPixmap(sorbonne_pixmap)
        sorbonne_logo.setAlignment(Qt.AlignRight)
        logo_layout.addWidget(sorbonne_logo, alignment=Qt.AlignRight)

        # Ajout du logo au layout principal
        header_layout.addLayout(logo_layout)

        # Titre centré (occupant tout l'espace horizontal)
        ...
        # Ajouter le header complet au layout principal
        ...
        detect_button = create_button("heatmap.png", "Détection Vidéo", "detection", 500, 500)
        detect_audio_button = create_button("audio_icon.png", "Détection Audio", "detection_audio", 500, 500)
...
class GenerationOptionPage(QWidget):
    ...
    def init_ui(self):
        ...
        image_label = QLabel()
        detect_audio_pixmap = QPixmap("generator.jpg").scaled(650, 650, Qt.KeepAspectRatio)
        if detect_audio_pixmap.isNull():
            print("Erreur : L'image 'generator.jpg' est introuvable.")
...
class GenerationVideoPage(QWidget):
    ...
    def init_ui(self):
        ...
        # Image
        image_label = QLabel()
        gener_pixmap = QPixmap("deepfake_generate.png").scaled(450, 450, Qt.KeepAspectRatio)
...
class DetectionVideoPage(QWidget):
    ...
    def init_ui(self):
        ...
        # Ajouter une image en dessous du titre
        image_label = QLabel()
        detect_pixmap = QPixmap("detect.jpg").scaled(650, 650, Qt.KeepAspectRatio)
...
class DetectionAudioPage(QWidget):
    ...
    def init_ui(self):
        ...
        # Ajouter une image en dessous du titre
        image_label = QLabel()
        detect_audio_pixmap = QPixmap("audio_main.png").scaled(850, 850, Qt.KeepAspectRatio)
        if detect_audio_pixmap.isNull():
