...
class HomePage(QWidget):
    ...
    def init_ui(self):
        ...
        generate_button = create_button("generation_v2.png", "Génération", "generation", 500, 500)
...
class DetectionPage(QWidget):
    ...
    def on_detection_finished(self, result):
        ...
        self.navigate_to("detection_result", score=confidence, classification=predicted_label)
...
class DetectionResultPage(QWidget):
    ...
    def init_ui(self):
        ...
        if self.classification is not None and self.score is not None:
            # Calcul du score ajusté via la logique transform()
            score = self.transform(self.score)

            # Calcul du pourcentage (texte)
            if self.classification.upper() == "REAL":
                percentage_score = (1 - score) * 100
            else:  # FAKE
            ...
            # smiley_pixmap = smiley_pixmap.scaled(64, 64, Qt.KeepAspectRatio, Qt.SmoothTransformation)
            ...
            probability_bar = ProbabilityBar(score, self.classification)
    ...
    def transform(self, x: float) -> float:
        """
        Transforme une valeur x dans l'intervalle [0,1] en amplifiant la séparation
        autour de la zone neutre [0.35, 0.65]
        """
        center = 0.5
        
        if x < center:
            if x >= 0.35:
                # Zone neutre basse : mapping [0.35, 0.5] -> [0.15, 0.5]
                return 0.15 + ((x - 0.35) / (0.5 - 0.35)) * (0.5 - 0.15)
            else:
                # Zone extrême basse : réduction de 15%
                return x * 0.85
        else:
            if x <= 0.65:
                # Zone neutre haute : mapping [0.5, 0.65] -> [0.5, 0.85]
                return 0.5 + ((x - 0.5) / (0.65 - 0.5)) * (0.85 - 0.5)
            else:
                # Zone extrême haute : augmentation de 15% de la distance à 1
                return x + ((1 - x) * 0.15)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        #self.detection_audio_result_page = DetectionAudioResultPage(self.navigate_to, self.go_back)
    ...
    def navigate_to(self, page, **kwargs):
        if page == "home":
            ...
            # Créer une nouvelle instance de DetectionResultPage avec les résultats
            ...
            score = kwargs.get("score", None)
            classification = kwargs.get("classification", None)
            # Créer une nouvelle instance de DetectionAudioResultPage avec les résultats
